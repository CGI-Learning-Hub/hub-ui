...
}>(({ selected, hasNoButtonOnFocus, theme }) => ({
  position: "absolute",
  top: 0,
  right: 0,
  width: "2rem",
...
export const ResourceCardBody = styled(CardContent)<{
  ...
  padding:
    size == "sm"
      ? ".7rem 5px .7rem 0 !important"
      : "0 16px 16px 16px !important",
  width: size === "sm" ? "calc(100% - 110px)" : "100%",
}));

export const ResourcePropertyItem = styled(Box)<{ size?: ResourceCardSize }>(
  ({ size }) => ({
    display: "flex",
    alignItems: "center",
    marginTop: "0.5rem",
...
export const ResourcePropertyText = styled(Box)<{
  size?: ResourceCardSize;
  width?: string;
  isLast: boolean;
}>(({ size, width, isLast }) => ({
...
export const ResourceIconItem = styled(Box)<{ size?: ResourceCardSize }>(
  ({ size }) => ({
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
...
export const PublicIconWrapper = styled(Box)<{ size?: ResourceCardSize }>(
  ({ size }) => ({
    position: "absolute",
